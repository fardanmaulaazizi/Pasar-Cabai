...
class KelompokTaniController extends Controller
{
    ...
    public function jsonKelompokTaniByDesa($id)
    {
        return DataTables::of(KelompokTani::with('user_info')->where('desa_id', $id)->get())
            ...
            ->addColumn('action', function ($row) {
                ...
                $action = '
                    <a class="btn btn-info" style="width:90px" href="' . $url . '">Detail</a>
                ';
    ...
    public function jsonKelompokTani()
    {
        return DataTables::of(KelompokTani::with('desa.kecamatan')->where('desa_id', Auth::user()->user_gudang->desa_id))
            ...
            ->addColumn('action', function ($row) {
                ...
                $action = '
                    <a class="btn btn-info mb-1" style="width:90px" href="kelompok-tani/' . $id . '">Detail</a>
                ';
                return $action;
            })
            ->rawColumns([
    ...
    {
        $desas = Desa::get();
        return view('user.kelompok-tani.index', compact(
            'desas'
        ));
    }
    ...
    public function store(Request $request)
    {
        ...
        $kelompokTani->desa_id = $request->desa_id;
